...
<section class="content-header">
      <h1>
        ...
        <small>Searched <?php echo $type;?></small>
      </h1>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
  <tbody>
    ...
    <?php
	  ...
	  $fsql = "select * from site_details_master where ";
	  if($type == 1){
	  	$fsql = $fsql." contract_date like '%$date'";
	  }
	  else{
	  	$fsql = $fsql." installation_date like '%$date%'";
	  }
	  $fsql = $fsql." order by site_id desc";
	 // echo $fsql;exit;
	  $frec = q($fsql);
	  $i = 1;
	  while($fres = f($frec))
    ...
    <tr align="center" >
      <td height="36" colspan="12">
        <table width="100%" border="0" cellspacing="1" cellpadding="1">
          <tr>
		  <form name="export" id="export" method="post" action="export_month_and_year_wise_list.php">
			<input type="hidden" name="sql" id="sql" value="<?php echo $fsql;?>" />
		  </form>
		  <form name="print" id="print" method="post" action="print_month_and_year_wise_list.php" target="_blank">
		  	<input type="hidden" name="sql" id="sql" value="<?php echo $fsql;?>" />
