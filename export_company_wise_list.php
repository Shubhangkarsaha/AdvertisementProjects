       ...
       $i=0;
		...
		while($fetch_emp = f($rec))
		{
          ...
          $objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('L'.($i+1), size_type($fetch_emp['cat_id'])) // error occures due to same name in the data base. have to use single single attribute selection method 
...
header('Content-Disposition: attachment;filename="Company Wise Report.xlsx"');
